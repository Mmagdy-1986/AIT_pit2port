(() => {
  ...
  const STORE_KEY = 'ait_pit2port_erp_mobile_reference_v1';
  ...
  function routeView() {
    ...
    let body = '';
    switch (state.route) {
      case 'companies': body = renderCompanies(); break;
      case 'companyDetail': body = renderCompanyDetail(); break;
      case 'equipments': body = renderEquipments(); break;
      case 'equipmentDetail': body = renderEquipmentDetail(); break;
      case 'contracts': body = renderContracts(); break;
      case 'operations': body = renderOperations(); break;
      case 'quality': body = renderQuality(); break;
      case 'consumption': body = renderConsumption(); break;
      case 'accounts': body = renderAccounts(); break;
      case 'notifications': body = renderNotifications(); break;
    ...
    const meta = pageMeta[state.route] || (state.route === 'companyDetail' ? ['تفاصيل الشركة', 'ملف الشركة والمعدات والماليات والتقارير.'] : ['تفاصيل المعدة', 'الحالة والعقد والتشغيل.']);
    if (state.route === 'home') return body;
    return `${screenHeader(meta[0], screenAction(state.route), screenBackRoute(state.route))}${body}`;
  }

  function statusBar() {
    return `<div class="statusbar"><span>9:41</span><div class="status-icons"><span>▮▮▮</span><span>⌁</span><i></i></div></div>`;
  }
  function screenHeader(title, action = '', backRoute = 'home') {
    return `${statusBar()}<div class="screen-header"><button class="back" data-route="${backRoute}" aria-label="رجوع">‹</button><div class="screen-title">${escapeHtml(title)}</div>${action || '<span class="ghost-action">.</span>'}</div>`;
  }
  function screenAction(route) {
    if (route === 'companies') return '<button class="add" data-modal="companyForm" aria-label="إضافة شركة">+</button>';
    if (route === 'equipments') return '<button class="add" data-modal="equipmentForm" aria-label="إضافة معدة">+</button>';
    if (route === 'contracts') return '<button class="add" data-modal="contractForm" aria-label="إضافة عقد">+</button>';
    return '';
  }
  function screenBackRoute(route) {
    if (route === 'companyDetail') return 'companies';
    if (route === 'equipmentDetail') return 'equipments';
    return 'home';
  }
  ...
  function renderHome() {
    const k = calcKpis();
    const currentMine = byId(state.data.mines, state.currentMineId).name;
    const today = new Intl.DateTimeFormat('ar-EG', { weekday:'long', day:'numeric', month:'long' }).format(new Date());
    return `<section class="panel home-screen">
      ${statusBar()}
      <div class="home-header">
        <button class="menu" data-route="more" aria-label="القائمة">☰</button>
        <div></div>
        <button class="bell" data-route="notifications" aria-label="التنبيهات">♢</button>
      </div>
      <div class="welcome">
        <div class="ait-logo">AIT</div>
        <h1>مرحباً بك في <strong>AIT Pit2Port</strong></h1>
        <p>نظام إدارة المعدات والمناجم</p>
        <div class="mutedline">${escapeHtml(currentMine)} · ${today}</div>
      </div>
      <div class="mine-hero" aria-hidden="true"></div>
      <div class="home-actions">
        ${moduleCard('▦','شركات المعدات','ملفات المؤجرين والمرفقات','companies')}
        ${moduleCard('⛟','المعدات','الأسطول والعقود والحالة','equipments')}
        ${moduleCard('▥','استهلاكات المعدات','وقود وزيوت وقطع غيار','consumption')}
        ${moduleCard('▧','التشغيل للمعدات','ورديات التشغيل والتوقفات','operations')}
        ${moduleCard('▤','الحسابات','الفواتير والمستحقات','accounts')}
      </div>
      <div class="quick-summary-title">ملخص سريع</div>
      <div class="mini-kpis">
        ${miniKpi(k.totalEquipments, 'المعدات العامة', '⛟')}
        ${miniKpi(k.maintenance, 'تحت الصيانة', '⌁')}
        ${miniKpi(k.runningContracts, 'العقود السارية', '▤')}
        ${miniKpi(k.endingSoon, 'استحقاقات قريبة', '♢')}
      </div>
    </section>`;
  }
  function miniKpi(value, label, icon) { return `<div class="mini-kpi"><strong>${value}</strong><span>${label}</span><i>${icon}</i></div>`; }
  function kpi(icon, label, value, sub, cls = '') { return `<article class="kpi ${cls}"><div class="kpi-head"><span>${label}</span><i class="kpi-icon">${icon}</i></div><strong>${value}</strong><small>${sub}</small></article>`; }
  ...
  function renderCompanies() {
    return `<section class="panel page">
      <div class="toolbar"><label class="search">⌕<input id="companySearch" placeholder="يبحث عن شركة..."></label></div>
      <div class="list">${state.data.companies.map((c,i)=>companyCard(c,i)).join('')}</div>
    </section>`;
  }
  function companyCard(c, i = 0) {
    const arabicNames = ['شركة النيل للمقاولات','شركة الأهرام للتعدين','شركة السويس للمعدات الثقيلة','شركة الوادي للمقاولات','شركة الصحراء للمعدات','شركة الدلتا للمعدات','شركة البحر الأحمر','شركة إيسترن ديزرت'];
    const icons = ['▥','▦','⛟','⌁','▣','▤','⛏','▧'];
    return `<article class="card entity" data-company-search="${escapeHtml([c.name,c.ownerName,c.commercialRegisterNumber, arabicNames[i]].join(' '))}" data-company-id="${c.id}">
      <div class="thumb">${icons[i % icons.length]}</div>
      <div class="entity-body"><h4>${escapeHtml(arabicNames[i] || c.name)}</h4><p>صاحب الشركة: ${escapeHtml(c.ownerName)}<br>${escapeHtml(c.ownerPhone)}</p></div>
    </article>`;
  }
  ...
  function renderEquipments() {
    const total = state.data.equipments.length;
    const running = state.data.equipments.filter(e=>e.status==='Running').length;
    const maintenance = state.data.equipments.filter(e=>e.status==='Maintenance').length;
    const stopped = state.data.equipments.filter(e=>e.status==='Stopped').length;
    return `<section class="panel page">
      <div class="toolbar"><label class="search">⌕<input id="equipmentSearch" placeholder="يبحث عن معدة..."></label></div>
      <div class="segmented"><button data-quick-filter-equipment="">الكل</button><button data-quick-filter-equipment="Running">نشطة</button><button data-quick-filter-equipment="Maintenance">صيانة</button><button data-quick-filter-equipment="Stopped">متوقفة</button></div>
      <div class="mini-kpis" style="margin-bottom:10px">${miniKpi(total,'إجمالي','⛟')}${miniKpi(running,'نشطة','✓')}${miniKpi(maintenance,'صيانة','⌁')}${miniKpi(stopped,'متوقفة','!')}</div>
      <div class="list">${state.data.equipments.slice(0, 60).map(equipmentCard).join('')}</div>
    </section>`;
  }
  function equipmentGlyph(type) {
    if (/Dump|Truck|Tanker/i.test(type)) return '🚚';
    if (/Loader|Bulldozer|Grader/i.test(type)) return '🚜';
    if (/Crusher/i.test(type)) return '⚙';
    return '⛏';
  }
  function equipmentCard(e) {
    const c = byId(state.data.companies, e.companyId);
    const statusText = e.status === 'Running' ? 'نشطة' : e.status === 'Maintenance' ? 'صيانة' : e.status === 'Stopped' ? 'متوقفة' : 'متاحة';
    const statusClass = e.status === 'Running' || e.status === 'Available' ? 'good' : e.status === 'Maintenance' ? 'warn' : 'bad';
    return `<article class="card entity equipment-row" data-equipment-search="${escapeHtml([e.name,e.code,e.status,statusText].join(' '))}" data-equipment-id="${e.id}">
      <div class="equipment-photo">${equipmentGlyph(e.type)}</div>
      <div class="entity-body"><h4>${escapeHtml(e.name)}</h4><p>${escapeHtml(e.code)}<br>${escapeHtml(c.name)}</p><div class="meta-row"><span class="chip ${statusClass}">${statusText}</span></div></div>
    </article>`;
  }
  ...
  function renderContracts() {
    const active = state.data.contracts.filter(c=>c.status==='Running').length;
    const ending = state.data.contracts.filter(c=>c.status==='Ending Soon').length;
    const expired = state.data.contracts.filter(c=>c.status==='Expired').length;
    return `<section class="panel"><div class="toolbar"><label class="search">⌕<input id="contractSearch" placeholder="بحث عقد / شركة / معدة"></label><button class="btn primary" data-modal="contractForm">+ عقد</button></div><div class="segmented"><button data-quick-filter-contract="Running">${active} Running</button><button data-quick-filter-contract="Ending Soon">${ending} Ending Soon</button><button data-quick-filter-contract="Expired">${expired} Expired</button><button data-quick-filter-contract="">PDF Ready</button></div><div class="list">${state.data.contracts.map(contractCard).join('')}</div></section>`;
  }
  ...
  function renderOperations() {
    return `<section class="panel page operation-page">
      <div class="section-title"><div><h3>بيانات التشغيل</h3><p>تسجيل وردية أو إيقاف تشغيل المعدة</p></div></div>
      <div class="card form-card">${shiftForm()}</div>
      <div class="section-title"><h3>آخر الورديات</h3></div>
      <div class="list">${state.data.shifts.slice(0, 12).map(shiftCard).join('')}</div>
    </section>`;
  }
  function shiftForm() {
    const eq = state.data.equipments[0];
    const driver = byId(state.data.drivers, eq.currentDriverId).name;
    const helper = byId(state.data.helpers, eq.currentHelperId).name;
    return `<form class="form" id="shiftForm"><div class="form-grid">
      <label>المعدة<select name="equipmentId">${state.data.equipments.map(e => `<option value="${e.id}">${e.name} (${e.code})</option>`).join('')}</select></label>
      <label>التاريخ<input name="date" type="date" value="${new Date().toISOString().slice(0,10)}"></label>
      <label>وقت البدء<input name="startTime" type="time" value="08:00"></label>
      <label>المنجم<select name="mineId">${state.data.mines.map(m => `<option value="${m.id}">${m.name}</option>`).join('')}</select></label>
      <label>الزون<select name="zoneId">${state.data.zones.map(z => `<option value="${z.id}">${z.name}</option>`).join('')}</select></label>
      <label>الصب زون<select name="subZoneId">${state.data.subZones.slice(0,40).map(s => `<option value="${s.id}">${s.name}</option>`).join('')}</select></label>
      <label>السائق<input value="${escapeHtml(driver)}" readonly></label>
      <label>التباع<input value="${escapeHtml(helper)}" readonly></label>
      <div class="section-title" style="margin:8px 0 2px"><h3>إيقاف التشغيل</h3></div>
      <label>وقت الإيقاف<input name="hours" type="number" value="8"></label>
      <label>سبب الإيقاف<select name="stopReason"><option>Maintenance</option><option>End Of Shift</option><option>Mechanical Failure</option><option>Waiting Truck</option><option>Waiting Material</option><option>Administrative</option><option>Weather</option><option>Fuel</option><option>Other</option></select></label>
      <label>ملاحظات<textarea name="notes" required placeholder="اكتب ملاحظات الإيقاف"></textarea></label>
    </div><div class="wide-actions"><button class="btn danger" type="button">إيقاف</button><button class="btn blue" type="submit">حفظ</button></div></form>`;
  }
  ...
  function renderQuality() {
    const lots = state.data.lots.slice(0, 18);
    return `<section class="panel page quality-page">
      <div class="section-title"><div><h3>معلومات الموقع</h3><p>إضافة لوط جديد وحساب الكمية تلقائياً</p></div></div>
      <div class="card form-card">${qualityForm()}</div>
      <div class="section-title"><h3>آخر اللوطات</h3></div>
      <div class="list">${lots.map(lotCard).join('')}</div>
    </section>`;
  }
  function qualityForm() {
    return `<form class="form" id="lotForm"><div class="form-grid">
      <label>الزون الرئيسي<select name="mineId">${state.data.mines.map(m => `<option value="${m.id}">${m.name}</option>`).join('')}</select></label>
      <label>صب زون<select name="zoneId">${state.data.zones.map(z => `<option value="${z.id}">${z.name}</option>`).join('')}</select></label>
      <label>الصب صب زون<select name="subZoneId">${state.data.subZones.map(s => `<option value="${s.id}">${s.name}</option>`).join('')}</select></label>
      <div class="section-title" style="margin:8px 0 2px"><h3>القياسات</h3></div>
      <label>المحيط (متر)<input name="baseCircumference" type="number" step="0.01" value="17.50"></label>
      <label>الارتفاع (متر)<input name="height" type="number" step="0.01" value="2.10"></label>
      <label>الكثافة (طن / م³)<input name="density" id="densityField" type="number" step="0.01" value="1.75" readonly></label>
      <div class="form-hint">تم تحديد الكثافة تلقائياً حسب نوع اللوط</div>
      <div class="section-title" style="margin:8px 0 2px"><h3>تفاصيل اللوط</h3></div>
      <label>مصدر اللوط<select name="lotType" id="lotType"><option>After Crusher</option><option>Before Crusher</option><option>Extraction</option></select></label>
      <div class="form-hint">حجم الحبيبات: 2 مم إلى 7 سم</div>
      <div class="quality-math"><div class="quality-result">56.27<br><small>م³</small></div><div class="quality-eq">=</div><div class="quality-result">98.98<br><small>طن</small></div><div class="quality-eq"></div><div class="quality-result">الكمية</div></div>
      <label>المعدة المرتبطة<select name="linkedEquipmentId">${state.data.equipments.filter(e=>e.status==='Running').map(e => `<option value="${e.id}">${e.code} - ${e.name}</option>`).join('')}</select></label>
      <label>ملاحظات<textarea name="notes" placeholder="اكتب ملاحظات (اختياري)"></textarea></label>
    </div><button class="btn primary" type="submit">حفظ اللوط</button></form>`;
  }
  function lotCard(l) {
  ...
  function onClick(e) {
    ...
    const reportCompany = e.target.closest('[data-report-company]')?.dataset.reportCompany;
    if (reportCompany) { state.reportType = 'company'; return navigate('reports'); }
    const equipmentQuick = e.target.closest('[data-quick-filter-equipment]')?.dataset.quickFilterEquipment;
    if (equipmentQuick !== undefined) { const input = $('#equipmentSearch') || $('#equipmentFilter'); if (input) input.value = equipmentQuick; filterCards('[data-equipment-search]', equipmentQuick); return; }
    const contractQuick = e.target.closest('[data-quick-filter-contract]')?.dataset.quickFilterContract;
    if (contractQuick !== undefined) { const input = $('#contractSearch'); if (input) input.value = contractQuick; filterCards('[data-contract-search]', contractQuick); return; }
